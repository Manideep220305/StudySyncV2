...
export const Sidebar = ({ children, open, setOpen, animate = true }: SidebarProviderProps) => {
  return (
...
export const DesktopSidebar = ({ className, children, ...props }: SidebarBodyProps) => {
  ...
  return (
    <motion.div
      className={cn(
        "h-full px-4 py-4 hidden md:flex md:flex-col bg-slate-900 w-[300px] flex-shrink-0 border-r border-slate-800",
        className
      )}
      animate={{
...
export const MobileSidebar = ({ className, children, ...props }: SidebarBodyProps) => {
  const { open, setOpen } = useSidebar();
  return (
    <>
      <div
        className={cn(
          "h-10 px-4 py-4 flex flex-row md:hidden items-center justify-between bg-slate-900 w-full border-b border-slate-800"
        )}
        {...props}
      >
